...
Next we are going to fill out the About Rule section. We are going to name the alert "SQL Injection Detected" and enter in a description for the rule. Set the "Default severity" to "High" then click on "Advance Settings". Scroll down to until you see "MITRE ATT&CK threats" and select the option for "Initial Access" for the technique and then add a subtechnique and set it to "Exploit Public-Facing Application".
